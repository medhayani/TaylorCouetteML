"""Train the CSON (Constrained Spectral Operator Network) surrogate.

Ensemble training: `n_seeds` independent runs of the same architecture
with different random seeds; at inference, take the median across seeds
for a robust prediction.

Loss = kink-weighted MSE
     + lam_spec  * spectral_penalty   (Chebyshev coefficients decay)
...
Output layout:
    ...
    out_root/test_branches.npz          test split + ctx normalisation stats
"""
...
def smoothness_penalty(pred: torch.Tensor) -> torch.Tensor:
    """Mean squared second difference of the prediction along k.

    Encourages local smoothness without over-smoothing the cusps because
    the data-driven kink-weighted MSE term already up-weights the loss
    near genuine cusps.
    """
...
def train_one_seed(model, train_ds, val_ds, out_dir, epochs, lr, batch, device,
    ...
    for ep in range(1, epochs + 1):
        model.train(); agg_total = 0.0; agg_data = 0.0; agg_spec = 0.0; agg_sm = 0.0; n = 0
        for b in tl:
            k = b["k_norm"].to(device); y = b["ta_norm"].to(device)
            ctx = b["ctx"].to(device)
            pred = model(k, ctx)
            L_data = kink_weighted_mse(pred, y)
            L_spec = model.spectral_penalty(ctx)
            ...
            agg_total += float(loss); agg_data += float(L_data)
        ...
        if ep == 1 or ep % 50 == 0 or ep == epochs:
            print(f"    ep {ep:4d}/{epochs}  "
                    f"data={agg_data/max(n,1):.5f}  spec={agg_spec/max(n,1):.4f}  "
                    f"smooth={agg_sm/max(n,1):.5f}  val_mse={v_mse:.5f}  val_mae={v_mae:.5f}",
                    flush=True)
...
def main():
    ...
    input_csv = ROOT / "data" / "combined_data_trainval.csv"          # leak-free: training and validation elasticities only
    desc_csv = ROOT / "data" / "branch_functional_descriptors_leakfree_trainval.csv"

    items = build_dataset(input_csv, desc_csv, None)
    ...
    for s_idx in range(args.n_seeds):
        ...
        print(f"CSON parameters: {n_params/1e6:.2f}M", flush=True)
