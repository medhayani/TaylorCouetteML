...
def load_rl_v2_and_windows(device, root):
    ...
    print(f"Loaded SARL_v2: {len(sarl_models)} seeds")
    ...
    print(f"Loaded MARL_v2: {len(marl_models)} seeds")
...
def main():
    ...
    for fig_idx, E in enumerate(Es):
        ...
        for _, row in sub_desc.iterrows():
            ...
            # Model 3: RL-v2 ensemble refiner
            rl = rl_v2_refine(sarl_models, marl_models, rl_idx, rl_arr,
                                  E, b_id, Ta_min, Ta_max, k_left, k_right, device)
            if rl is not None:
                k_rl, Ta_rl = rl
                ax.plot(k_rl, Ta_rl, ".", ms=4, color="red",
                          label="_nolegend_", zorder=17)

        n_sup = 12 + 5 + 5 + 7 + 3 + 2
        n_rl = len(sarl_models) + len(marl_models)
        ...
        ax.plot([], [], ".", ms=8, color="red",
                  label=f"RL-v2  (median of {n_rl} refiners: SARL_v2 5 seeds + MARL_v2 5 seeds)")
